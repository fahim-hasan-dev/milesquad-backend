...
const getMyEarningsSummary = async (driverId: string, query: Record<string, unknown>) => {
    ...
    const fromDateStr = query.fromDate ? String(query.fromDate) : "today";
    const toDateStr = query.toDate ? String(query.toDate) : "today";

    return getOrSetCache(
        `cache:driver:earnings:${driverId}:${fromDateStr}:${toDateStr}`,
        async () => {
            const id = new Types.ObjectId(driverId);
